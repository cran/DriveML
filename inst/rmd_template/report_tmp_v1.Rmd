...
```{r mdp1,warning=FALSE,eval=T,render=t1,echo=F}
...
```{r mdp2,warning=FALSE,eval=T,render=t1,echo=F}
...
cc  <- mldata$raw_list$Num_Correlation
paged_table(cc)
```
...
t1 <- t1[t1$Obs!=0,]; rownames(t1) <- NULL
kable(t1)
```
...
t2 <- t2[t2$Obs!=0,]; rownames(t2) <- NULL
kable(t2)
```
...
if(!is.null(t3)) {
  t3 <- t3[t3$Obs!=0,]; rownames(t3) <- NULL
  kable(t3)
} else {
